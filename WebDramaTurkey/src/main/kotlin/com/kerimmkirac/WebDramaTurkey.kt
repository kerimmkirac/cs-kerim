...
class WebDramaTurkey(sharedPref: SharedPreferences? = null) : MainAPI() {
    override var mainUrl = getDomain(sharedPref, "WebDramaTurkey")
    ...
    companion object {
        private const val CACHE_DURATION = 5 * 60 * 1000L // 5 dakika
        ...
        private fun getDomain(sharedPref: SharedPreferences?, providerName: String): String {
            return runBlocking {
                try {
                    ...
                    // 5 dakika geçmediyse direkt SharedPreferences'ten al
                    if (now - lastUpdate < CACHE_DURATION) {
                        return@runBlocking sharedPref?.getString(providerName, "") ?: ""
                    }

                    // 5 dakika geçtiyse güncelle
                    val domainListesi =
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val document =
            app.get("${request.data}${if (request.data == mainUrl + "/") "" else "?page=$page"}").document
    ...
    private fun Element.toLatestEpisodeResult(): SearchResponse? {
        ...
        if (posterUrl.isNullOrEmpty()) {
            this.selectFirst("div.media.media-episode")?.attr("style")?.let { style ->
                val urlRegex =
                    """https://[^"'\s)]+\.(jpg|jpeg|png|webp)""".toRegex(RegexOption.IGNORE_CASE)
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val movDescription =
            document.selectFirst("div.video-attr:nth-child(4) > div:nth-child(2)")?.text()?.trim()
        val year = document.selectFirst("div.featured-attr:nth-child(1) > div:nth-child(2)")?.text()
            ?.trim()?.toIntOrNull()
        val movYear =
            document.selectFirst("div.video-attr:nth-child(3) > div:nth-child(2)")?.text()?.trim()
                ?.toIntOrNull()
    ...
    override suspend fun loadLinks(
        ...
        val TAG = "AYZEN_DEBUG"
        Log.d(TAG, "loadLinks başladı: $data")
        ...
        Log.d(TAG, "Bulunan Embed ID Sayısı: ${embedIds.size} ($embedIds)")

        if (embedIds.isEmpty()) {
            Log.w(TAG, "Hiç embed ID bulunamadı!")
        }

        for (id in embedIds) {
            Log.d(TAG, "İşleniyor Embed ID: $id")
            ...
            val response = app.post(
                ...
                headers = mapOf(
                    ...
                    "User-Agent" to "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:137.0) Gecko/20100101 Firefox/147.0"
                ),
                data = mapOf("id" to id)
            ).document

            val iframeUrl = response.selectFirst("iframe")?.attr("src")
            if (iframeUrl == null) {
                Log.e(TAG, "ID $id için iframeUrl bulunamadı!")
                continue
            }

            Log.d(TAG, "Iframe URL alındı: $iframeUrl")

            val iframeGet = app.get(iframeUrl, referer = data).document

            val mainIframe = iframeGet.selectFirst("#main-iframe")?.attr("src")
                ?: iframeGet.selectFirst("iframe")?.attr("src") ?: ""

            if (mainIframe.isNotBlank()) {
                if (mainIframe.contains("#")) {
                    // Özel API Yapısı (Hash'li ID ayıklama)
                    val videoId = mainIframe.substringAfterLast("#")
                    val base = mainIframe.substringBefore("#").trimEnd('/')
                    val finalApiUrl = "$base/api/v1/info?id=$videoId"

                    Log.i(TAG, "Özel API Tespit Edildi, Yönlendiriliyor: $finalApiUrl")
                    loadExtractor(finalApiUrl, iframeUrl, subtitleCallback, callback)
                } else {
                    // Standart Servisler (Vidmoly, VK, Odnoklassniki vb.)
                    Log.i(TAG, "Standart Extractor Tespit Edildi (Vidmoly/VK vb.): $mainIframe")

                    // Vidmoly linkini direkt gönderiyoruz, Cloudstream kendisi çözecek.
                    loadExtractor(mainIframe, iframeUrl, subtitleCallback, callback)
                }
            } else {
                Log.e(TAG, "HATA: mainIframe boş döndü! ID: $id")
            }
        }
        return true
    }
