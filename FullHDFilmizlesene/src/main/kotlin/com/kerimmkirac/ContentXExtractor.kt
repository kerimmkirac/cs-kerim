...
// DÜZELTİLMİŞ REGEX (Unicode ve özel karakterleri düzgün yakalıyor)
        ...
        Log.d("kraptor_$name", "subtitle » $subUrls")
...
open class TRsTX : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        Log.d("kerim_${this.name}", "postJson » $postJson")
        ...
        for (mapEntry in vidMap) {
            Log.d("kerim_${this.name}", "mapEntry » $mapEntry")
...
open class TurboImgz : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        Log.d("kerim_${this.name}", "videoLink » $videoLink")
...
open class VidMolyExtractor : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        if (answer != ""){
            ...
            val matches = Regex("""file\s*:\s*"([^"]+\.m3u8[^"]*)"""").findAll(ilkYanit.text).toList()
            if (matches.isEmpty()) {
                Log.w("kraptor_$name", "Vidmoly'de m3u8 link bulunamadı")
                return
            }
