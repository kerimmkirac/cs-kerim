...
class SetFilmIzle(sharedPref: SharedPreferences? = null) : MainAPI() {
    override var mainUrl              = getDomain(sharedPref, "SetFilmIzle")
        ...
        private fun getDomain(sharedPref: SharedPreferences?, providerName: String): String {
            return runBlocking {
                try {
                    ...
                    val domainListesi = app.get("https://raw.githubusercontent.com/Kraptor123/domainListesi/refs/heads/main/eklenti_domainleri.txt").text
...
override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
    ...
    document.select("nav.player a").map { element ->
        ...
        if (sourceIframe.contains("setplay.shop") || sourceIframe.contains("setplay.site") || sourceIframe.contains("setplay.cfd")) {
            val finalUrl = if (sourceIframe.contains("?")) {
